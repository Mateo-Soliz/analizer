...
export default function AnalysisTable({
  ...
  const filteredAndSortedData = data
    .filter((row) => {
      const matchesSearch = row.group.toLowerCase().includes(searchTerm.toLowerCase())
      const matchesFilter =
  ...
  const avgAmplitude = data.reduce((sum, row) => sum + row.amplitude, 0) / data.length
  ...
  const exportData = () => {
    const csv = [
      ["Grupo", "Mesor", "Amplitud", "Acrofase", "Hora Pico", "MSE", "R²", "Chi²", "F-Statistic", "P-valor"],
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="text-center p-4 bg-muted/50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{significantCount}</div>
              <div className="text-sm text-muted-foreground">Significativos (p&lt;0.05)</div>
            </div>
            <div className="text-center p-4 bg-muted/50 rounded-lg">
              <div className="text-2xl font-bold text-purple-600">{avgR2.toFixed(2)}</div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Select value={filterSignificant} onValueChange={setFilterSignificant}>
              ...
              <SelectContent>
                ...
                <SelectItem value="non-significant">No significativos</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </CardContent>
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>
                    <Button variant="ghost" onClick={() => handleSort("group")} className="h-auto p-0 font-semibold">
                      Grupo
                  ...
                  <TableHead>
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button variant="ghost" onClick={() => handleSort("r2")} className="h-auto p-0 font-semibold">
                            R²
                            <Info className="ml-1 h-3 w-3" />
                          </Button>
                        </TooltipTrigger>
                  ...
                  <TableHead>
                    <Button variant="ghost" onClick={() => handleSort("p_value")} className="h-auto p-0 font-semibold">
                      Significancia
                      <ArrowUpDown className="ml-2 h-4 w-4" />
                    </Button>
                  </TableHead>
              ...
              <TableBody>
                {filteredAndSortedData.map((row) => (
                  <TableRow key={row.group} className="hover:bg-muted/50">
                    ...
                    <TableCell className="font-mono">{formatPeakTime(row.peak_time)}</TableCell>
                    <TableCell>
                      <span className={getR2Color(row.r2)}>{row.r2?.toFixed(3) ?? "-"}</span>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        {getSignificanceBadge(row.p_value)}
                        <span className="text-xs text-muted-foreground">{row.p_value?.toExponential(2)}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => copyToClipboard(row.group)}>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex flex-wrap items-center gap-4">
            <span className="text-sm font-medium">Leyenda de significancia:</span>
